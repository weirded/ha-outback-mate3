...
- [x] **R10** — `docs/superpowers/specs/2026-04-17-ha-addon-design.md` was stale vs the implementation. _(2.0.0 — spec rewritten to match what shipped. Status now says Implemented. Architecture diagram updated with WS 28099, HTTP config poll, and the bundled-integration deploy. Protocol section drops `aggregates` / `aggregates_updated`, adds `config_snapshot`, replaces hand-rolled ping/pong with aiohttp heartbeat, documents the bounded-queue + per-client-isolation broadcast semantics. Repository layout + component-design sections reconciled with the actual files. Two "Changed from proposal" callouts explain the material differences (aggregates moved to the integration; aiohttp heartbeat replaced hand-rolled ping/pong) so future contributors see the reasoning.)_ 