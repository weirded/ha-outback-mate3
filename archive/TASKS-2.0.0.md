...
- [x] **17.8** Add `outback_mate3_addon/logo.png` (512×512 PNG). Upscaled from `icon.png` via ImageMagick. _(2.0.0-dev22)_
...
- [x] **18.2** Add snapshot tests via `syrupy` for entity state. `tests/__snapshots__/test_integration.ambr` captures entity_id/state/unit/device_class/state_class for every `sensor.mate3_*` and `binary_sensor.mate3_*` after the standard SNAPSHOT_PAYLOAD fixture is processed; regenerate with `pytest --snapshot-update`. _(2.0.0-dev22)_
- [x] **18.3** ~~Add `async_step_reauth(self, entry_data)` to config_flow.~~ N/A — the integration has no authentication (WS is unauthenticated over the Supervisor-internal network). HA's quality scale `reauthentication-flow` rule is explicitly conditional on the integration requiring auth; adding a placeholder step today is dead code that violates the "don't design for hypothetical future requirements" rule in CLAUDE.md. Will land the day we introduce auth, not before. _(decided 2026-04-18)_
- [x] **18.4** Finish B20 — MyPy strict across both components, wire into `.github/workflows/lint.yaml`. MyPy config now lives in `pyproject.toml` under `[tool.mypy]` with `explicit_package_bases = true` and `mypy_path = "outback_mate3_addon"` so the add-on's `src.*` imports resolve; `pytest_homeassistant_custom_component.*` and `pytest_socket` get `ignore_missing_imports` overrides. Real fixes — not just config — in the integration: `FlowResult` → `ConfigFlowResult`, `DeviceInfo` import moved from `homeassistant.helpers.entity` to `homeassistant.helpers.device_registry`, `CoordinatorEntity[OutbackMate3]` parameterized (matches `DataUpdateCoordinator[None]`), `entities: list[SensorEntity]` annotated, `native_value` return type pinned to `StateType`, division cast to `float`. CI's `lint.yaml` runs mypy as two separate invocations (`custom_components/outback_mate3 tests` and `outback_mate3_addon/src outback_mate3_addon/tests`) because both `tests/` dirs have a `conftest.py` and mypy can't merge them into one module path. _(2.0.0-dev21)_
- [x] **18.5** Build user-facing `docs/` (architecture, troubleshooting, FAQ) beyond the README. Three new files: `docs/architecture.md` (why-not-what — the split-halves rationale, WS-over-HTTP/TCP choice, bounded-queue design, Supervisor-trust security model, CONFIG.xml discovery, repairs design, single-version coupling), `docs/troubleshooting.md` (symptom-driven — missing discovery card, Connected=Off, Unavailable sensors, both Repairs issues, Energy Dashboard skip, enabling diagnostic sensors, downloading diagnostics, tailing add-on logs, CONFIG.xml unreachable), `docs/faq.md` (running without the add-on, HA Container, multi-MATE3, FNDC, update cadence, 5-min staleness, HACS drop, 1.x migration, bug-report checklist, contributing, MIT licensing). README's "Troubleshooting" + new "Further reading" section link all three. Intentionally kept WHY-focused rather than re-describing implementation, to avoid the parallel-truth rot that killed the earlier design spec (PR #6 R10). _(2.0.0-dev22)_
- [x] **18.6** Add `.github/workflows/release.yaml` — tag-triggered release that validates version bump across add-on + integration. Fires on `v*.*.*` and `v*.*.*-*` tags: first job strips the leading `v` and aborts the release if `outback_mate3_addon/config.yaml` or `custom_components/outback_mate3/manifest.json` disagrees with the tag; second job awk-extracts the matching `## <version> — <date>` section from `CHANGELOG.md` and publishes via `softprops/action-gh-release@v2` with `prerelease: true` auto-set on any `-devN` tag. _(2.0.0-dev22)_
- [x] **18.7** Audit the integration against HA's gold-tier quality-scale checklist. Added `custom_components/outback_mate3/quality_scale.yaml` that walks every bronze / silver / gold / platinum rule with `done` / `todo` / `exempt` status. Initially revealed one gold gap: `entity-translations` on the ~400+ sensors. See 18.8 for the fix that closed it. _(2.0.0-dev23)_
- [x] **18.8** Finish `entity-translations` for gold: replaced `self._attr_name = name` with `self._attr_translation_key = sensor_type` in `OutbackBaseSensor` (`sensor.py`) and `self._attr_translation_key = key` in `OutbackConfigDiagnosticSensor` (`sensor_config.py`). Wrote a one-shot extractor that harvested 262 unique `(translation_key → display_name)` pairs from the live-sensor call sites and the `_SYSTEM_/_INVERTER_/_CC_CONFIG_SENSORS` tables (including the 57 programmatically-generated Grid_Use TOU entries), resolved 4 inverter/CC naming overlaps by preferring the shorter CC-style names ("Absorb Voltage" over "Charger Absorb Voltage" etc.), and emitted `entity.sensor.<key>.name` entries into both `strings.json` and `translations/en.json`. Kept the `name` positional argument on both sensor constructors as the readable source of truth alongside each `sensor_type` at call sites — strings.json is derived from it, not the other way around. All 27 integration tests + existing snapshot still pass. Flipped `manifest.json` → `"quality_scale": "gold"` and marked `entity-translations: done` in `quality_scale.yaml`. _(2.0.0-dev23)_
...
- [x] **R11** — `custom_components/outback_mate3/__init__.py:389`'s `_apply_device` used unchecked `payload["mac"]` / `["kind"]` / `["index"]` / `["state"]` subscripts, unlike the defensive `.get()` pattern in `_apply_config`. A malformed `device_added` / `state_updated` / `snapshot` device entry would raise `KeyError` out of `_apply_device` → `_handle_message` → `_consume` → `_ws_loop`, whose handler only catches `(aiohttp.ClientError, TimeoutError, OSError)`. The fire-and-forget task would die permanently; the integration went silent until HA restart. _(2.0.0-dev1 — wrapped `self._handle_message(payload)` in `_consume` with `except Exception` that logs (via `_LOGGER.exception`) and continues, so the WS boundary swallows any handler oversight uniformly. New regression test `test_malformed_device_payloads_do_not_kill_ws_loop` sends four malformed device frames before a valid snapshot and asserts entities still materialize.)_
- [x] **R12** — Bundle of polish items from Copilot's second review pass: `scripts/onboard-haos.sh` minted a long-lived token via an embedded Python snippet that imports `aiohttp`, but the preflight only checked `jq` and `curl`; `tests/conftest.py` and `outback_mate3_addon/tests/conftest.py` imported `pycares` unconditionally, so any future drift where it stops being a transitive HA dep would take the whole suite down at collection; and `CLAUDE.md`'s Project-layout entry + Standing-rule step 3 still referenced a root-level `TASKS.md` that this PR archived to `archive/TASKS-2.0.0.md`. _(2.0.0-dev2 — `onboard-haos.sh` now preflights `python3` and `python3 -c 'import aiohttp'` alongside jq/curl with a clear install hint; both conftests wrap the `pycares` import in `try/except ImportError` (no pycares → no pycares thread → nothing to warm); `CLAUDE.md` points at `archive/TASKS-2.0.0.md` with a note that a fresh root-level `TASKS.md` opens at the next release cycle, and the Standing rule now talks about "the active task log" instead of a hardcoded path. Skipped Copilot's ws_server handshake-race flag for this release — staleness is bounded by the 30 s per-MAC throttle and a naive fix introduces concurrent `ws.send_json` on the same socket; a proper per-client send-lock is follow-up work, not 2.0 scope.)_

## Release cut — 2.0.0

- [x] **C1** Final version bump `2.0.0-dev23` → `2.0.0` (drop `-dev` suffix) across `outback_mate3_addon/config.yaml` and `custom_components/outback_mate3/manifest.json`. _(2.0.0)_
- [x] **C2** Apply deferred `ruff format` sweep (22 files, whitespace-only) and enable `ruff format --check` in `.github/workflows/lint.yaml`. This closes the gate the format-check had been disabled behind since 16.16. _(2.0.0)_
- [x] **C3** Delete `.github/workflows/hacs.yaml` — HACS distribution was removed in 2.0; the workflow was running validation against a category we no longer ship. _(2.0.0)_
